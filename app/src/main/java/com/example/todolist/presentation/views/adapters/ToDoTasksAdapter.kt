package com.example.todolist.presentation.views.adapters
...
class ToDoTaskAdapter(
    ...
    fun setRecyclerviewDataset(newToDoTasks: List<ToDoTask>?) {
        val tempToDoTasks = arrayOfNulls<ToDoTask>(DateUtility.HOURS_IN_DAY).toMutableList()
        newToDoTasks?.forEach{ toDoTask ->
            val position = dateUtility.getTimeHours(toDoTask.toDoTaskBegin)
